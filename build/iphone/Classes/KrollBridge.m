...
NSString * PMXFramwork$ModuleRequireFormat = @"(function(exports){"
...
@implementation PMXFramworkObject
...
{
	// create PMXFramwork global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "PMXFramwork" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_pmxframwork = [[PMXFramworkObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_pmxframwork];
	
	NSString *_pmxframworkNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _pmxframworkNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_pmxframwork valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:PMXFramwork$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.pmxframwork.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.pmxframwork.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
