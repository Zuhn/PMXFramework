...
// in simulator we redefine to format for PMXFramwork Developer console
...
{\
	NSString * exceptionName = [@"org.pmxframwork." stringByAppendingString:NSStringFromClass([self class])];\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
\
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.pmxframwork";\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
...
enum {
	UIPMXFramworkNativeItemNone = -1, 
	UIPMXFramworkNativeItemSpinner = -2,
	UIPMXFramworkNativeItemProgressBar = -3,
	
	UIPMXFramworkNativeItemSlider = -4,
	UIPMXFramworkNativeItemSwitch = -5,
	UIPMXFramworkNativeItemMultiButton = -6,
	UIPMXFramworkNativeItemSegmented = -7,
	
	UIPMXFramworkNativeItemTextView = -8,
	UIPMXFramworkNativeItemTextField = -9,
	UIPMXFramworkNativeItemSearchBar = -10,
	
	UIPMXFramworkNativeItemPicker = -11,
	UIPMXFramworkNativeItemDatePicker = -12,
	
	UIPMXFramworkNativeItemInfoLight = -13,
	UIPMXFramworkNativeItemInfoDark = -14,
	
	UIPMXFramworkNativeItemDisclosure = -15,
	
	UIPMXFramworkNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all PMXFramwork instances of
