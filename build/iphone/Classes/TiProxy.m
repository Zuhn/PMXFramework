...
//Rare exceptions to indicate a bug in the _pmxframwork code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_pmxframwork = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_pmxframwork]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.pmxframwork.module"
